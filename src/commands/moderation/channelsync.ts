...
export default class ChannelSync implements CommandInterface {
    ...
    public description =
        'Sincronize a configuração do seu canal com a configuração da categoria que ele se encontra';
    ...
    public async run(client: Client, message: Message, args: string[]) {
        ...
        const userHasPermission = message.member?.hasPermission(
            ['ADMINISTRATOR'],
            { checkOwner: true, checkAdmin: true }
        );
        ...
        if (!channel.parent)
            return sendErrorMessage(
                { errorMessage: 'O canal não possui uma categoria' },
                message
            );
