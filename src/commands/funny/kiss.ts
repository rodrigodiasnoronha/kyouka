...
export default class Kiss implements CommandInterface {
    ...
    public async run(message: Message, args: string[]) {
        ...
        const gif =
            images.kissGif[Math.floor(Math.random() * images.kissGif.length)];
        const messageEmbed = new MessageEmbed().setImage(gif);

        if (!user) {
            return sendErrorMessage(
                {
                    errorMessage: `você não pode beijar ninguém! Mencione um usuário! :wink: `,
                },
                message
            );
        }
        ...
        messageEmbed.setDescription(
            `<@${message.author}> beijou <@${user}>! Shippo demais!`
        );
